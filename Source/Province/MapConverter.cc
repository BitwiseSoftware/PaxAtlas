...
void MapConverter::init_tiles() {
  provinces_image.loadFromFile(
    Mikan::Engine::ROOT_DIR + "Resources/Map/Provinces.bmp"
  );
  ...
  for (auto& pair : tiles) {
    ...
    while (true) {
      auto found = std::find_if(
          ...
          [last_pixel](const sf::Vector2f& pixel) {
            const bool same_row_ahead 
                = pixel == last_pixel + sf::Vector2f(1, 0);
            const bool same_column_ahead
                = pixel == last_pixel + sf::Vector2f(0, 1);
            const bool same_row_behind
                = pixel == last_pixel - sf::Vector2f(1, 0);
            const bool same_column_behind
                = pixel == last_pixel - sf::Vector2f(0, 1);
            if (same_row_ahead || same_column_ahead ||
                same_row_behind || same_column_behind) {
              return true;
            } else {
              return false;
            }
